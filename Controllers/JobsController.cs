...
//            if (date.HasValue)
//            {
//                return Ok(await _service.GetJobsByDate(date.Value));
//            }
...
//        if (date.HasValue)
//        {
//            // 🔥 MUST call this (NOT user filter)
//            return Ok(
...
GetAll([FromQuery] DateTime? date, [FromQuery] bool? today)
    {
        ...
        // 🔥 today=true → convert to today's date
        if (today == true)
        {
            date = DateTime.Today;
        }
        ...
        if (date.HasValue)
        {
            // 🔥 IMPORTANT → show ALL today's jobs
            return Ok(await _service.GetJobsByDate(date.Value));
        }

        // 🔥 only own jobs
        return Ok(await _service.GetJobsByUser(userId));
    }
    ...
    Update(int id, JobDto dto)
    {
        ...
        // 🔥 FIXED: use PhotographerId
        if (!User.IsInRole("Admin") &&
            job.PhotographerId != userId)
        {
            return StatusCode(403, new
            {
                message = "You can update only assigned jobs"
            });
        }
